...
import { existsSync as existsSync3, readFileSync as readFileSync3, readdirSync, statSync, writeFileSync as writeFileSync2 } from "node:fs";
import { basename, dirname, isAbsolute as isAbsolute2, join as join2, resolve, sep } from "node:path";
...
function splitFrontmatter(src) {
  ...
  const block = match[1] ?? "";
  return { data: yaml.load(block) ?? {}, content: src.slice(consumed.length) };
}
...
function readFrontmatterFile(file, die2) {
  ...
  try {
    return splitFrontmatter(readFileSync2(file, "utf8"));
  } catch (err) {
    return die2(`invalid frontmatter in ${file}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
function writeFrontmatterFile(file, data, content) {
  const out2 = Object.keys(data).length === 0 ? content : `---
${dumpBlock(data)}---
${content}`;
  writeFileSync(file, out2);
}
...
var GIT_IDENTITY = ["-c", "user.name=strapped", "-c", "user.email=strapped@localhost"];
function git(cwd, ...args) {
  return spawnSync("git", ["-C", cwd, ...args], { encoding: "utf8" });
}
function stateRootFromPath(target) {
...
function autoCommitStateRoot(target, message) {
  try {
    const stateRoot = stateRootFromPath(target);
    if (stateRoot === null || !existsSync3(join2(stateRoot, ".git")))
      return;
    git(stateRoot, "add", "-A");
    git(stateRoot, ...GIT_IDENTITY, "commit", "-q", "-m", message);
  } catch {}
}
function cmdCommit(arg, message) {
  let stateRoot;
  if (arg !== null) {
    const derived = stateRootFromPath(arg);
    if (derived === null)
      die2(`cannot derive stateRoot from "${arg}" (no runs/ ancestor)`);
    stateRoot = derived;
  } else {
    stateRoot = resolveStateRoot(die2);
  }
  let initialized = false;
  if (!existsSync3(join2(stateRoot, ".git"))) {
    const init = git(stateRoot, "init", "-q", "-b", "main");
    if (init.error)
      die2(`git is not available: ${init.error.message}`);
    if (init.status !== 0)
      die2(`git init failed in ${stateRoot}: ${init.stderr.trim()}`);
    initialized = true;
  }
  const add = git(stateRoot, "add", "-A");
  if (add.error)
    die2(`git is not available: ${add.error.message}`);
...
function cleanupWorktree(file, onError) {
  ...
  if (!existsSync3(manifestFile))
    onError(`no manifest at ${manifestFile} (cannot resolve repo root)`);
  const mfm = parseFrontmatter(readFileSync3(manifestFile, "utf8"), manifestFile, onError);
  const repo = data.repo ?? null;
  const repos = Array.isArray(mfm.repos) ? mfm.repos : [];
...
function cmdCleanup(file) {
  out(cleanupWorktree(file, die2));
}
function cmdOutstanding(runDirArg) {
  const stateRoot = resolveStateRoot(die2);
  let runDirs;
  if (runDirArg !== null) {
    runDirs = [runDirArg];
  } else {
    const runsRoot = join2(stateRoot, "runs");
    runDirs = existsSync3(runsRoot) ? readdirSync(runsRoot).map((name) => join2(runsRoot, name)).filter((dir) => statSync(dir).isDirectory()).sort() : [];
  }
  const deliverables = [];
  for (const runDir of runDirs) {
    ...
    for (const listEntry of list) {
      ...
      const dfile = isAbsolute2(fileField) ? fileField : join2(runDir, fileField);
      if (!existsSync3(dfile))
        continue;
      const fm = parseFrontmatter(readFileSync3(dfile, "utf8"), dfile, die2);
      if (fm.status === "merged")
        continue;
...
var GITHUB_RECONCILABLE = new Set(["unaddressed", "addressed"]);
function feedbackIndexPath(runDir) {
  return join2(runDir, "feedback-index.json");
}
...
function readFeedbackIndex(path) {
  if (!existsSync3(path))
  ...
  try {
    ...
    die2(`invalid JSON in feedback index ${path}`);
  }
  if (typeof parsed !== "object" || parsed === null)
    die2(`malformed feedback index ${path}`);
  const obj = parsed;
  const version = typeof obj.version === "number" ? obj.version : 1;
  const comments = Array.isArray(obj.comments) ? obj.comments : [];
  return { version, comments };
}
function writeFeedbackIndex(path, index) {
  writeFileSync2(path, JSON.stringify(index, null, 2) + `
`);
}
function cmdFeedbackIndexRead(runDir) {
...
function cmdFeedbackIndexUpsert(runDir, fromFile) {
  if (!existsSync3(fromFile))
    die2(`--from file not found: ${fromFile}`);
  let incoming = null;
  try {
    incoming = JSON.parse(readFileSync3(fromFile, "utf8"));
  } catch {
    die2(`invalid JSON in --from file ${fromFile}`);
  }
  if (!Array.isArray(incoming))
    die2(`--from file must contain a JSON array of comments: ${fromFile}`);
  const path = feedbackIndexPath(runDir);
  const index = readFeedbackIndex(path);
  const byId = new Map(index.comments.map((c) => [c.externalId, c]));
  ...
  for (const rec of incoming) {
    ...
    if (externalId === null)
      die2(`incoming comment missing externalId in ${fromFile}`);
...
function cmdFeedbackIndexSet(runDir, externalId, status, commit) {
  if (!FEEDBACK_STATUSES.has(status))
    die2(`unknown feedback status "${status}"`);
  const path = feedbackIndexPath(runDir);
  if (!existsSync3(path))
    die2(`no feedback index at ${path}`);
  const index = readFeedbackIndex(path);
  const comment = index.comments.find((c) => c.externalId === externalId);
  if (comment === undefined)
    die2(`unknown externalId "${externalId}" in ${path}`);
...
var USAGE = "usage: state.mjs <resolve|dag|set|transition|manifest-status|feedback-index|commit|cleanup|outstanding> ...";
...
switch (cmd) {
  ...
  case "feedback-index": {
    ...
    switch (sub) {
      case "read": {
        ...
        if (!runDirArg)
          die2("usage: state.mjs feedback-index read <runDir>");
      ...
      case "upsert": {
        ...
        const runDirArg = rest[1];
        if (!runDirArg || !from)
          die2("usage: state.mjs feedback-index upsert <runDir> --from <jsonFile>");
      ...
      case "set": {
        ...
        if (!runDirArg || externalId === undefined || status === undefined) {
          die2("usage: state.mjs feedback-index set <runDir> <externalId> <status> [--commit <sha>]");
        }
        cmdFeedbackIndexSet(resolve(runDirArg), externalId, status, commit);
        break;
      }
      default:
        die2("usage: state.mjs feedback-index <read|upsert|set> ...");
    }
    break;
  }
  case "commit": {
  ...
  case "cleanup": {
    const fileArg = rest[0];
    if (!fileArg)
      die2("usage: state.mjs cleanup <deliverableFile>");
