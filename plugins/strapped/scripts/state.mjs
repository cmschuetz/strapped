...
import { basename, isAbsolute as isAbsolute2, join as join2, resolve } from "node:path";
...
function git(stateRoot, args, { allowFail = false } = {}) {
  const res = spawnSync("git", ["-C", stateRoot, ...args], { encoding: "utf8" });
  if (res.error)
    die2(`git ${args.join(" ")} failed to spawn: ${res.error.message}`);
  if (!allowFail && res.status !== 0) {
    die2(`git ${args.join(" ")} failed: ${(res.stderr || "").trim()}`);
  }
  return { status: res.status, stdout: res.stdout ?? "", stderr: res.stderr ?? "" };
}
function cmdSnapshot(runDir, message) {
  const stateRoot = resolveStateRoot(die2);
  const msg = message || `strapped snapshot ${basename(runDir)}`;
  if (!existsSync3(join2(stateRoot, ".git")))
    git(stateRoot, ["init", "-b", "main", "--quiet"]);
  git(stateRoot, ["add", "-A"]);
  ...
  const idArgs = email.status === 0 && email.stdout.trim() !== "" ? [] : ["-c", "user.name=strapped", "-c", "user.email=strapped@localhost"];
...
function tryParseFrontmatter(file) {
...
`);
  if (lines[0] !== "---" || lines.indexOf("---", 1) === -1)
    return null;
  try {
    return parseFrontmatter(src, file, (m) => {
      throw new Error(m);
    });
  } catch {
    return null;
...
function listRunDirs(stateRoot, slug) {
  const runsRoot = join2(stateRoot, "runs");
  if (!existsSync3(runsRoot))
    return [];
  if (slug !== null) {
    const dir = join2(runsRoot, slug);
    return existsSync3(dir) ? [dir] : [];
  }
  return readdirSync(runsRoot, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => join2(runsRoot, d.name)).sort();
}
function reposRootMap(runDir) {
  const manifest = join2(runDir, "manifest.md");
  if (!existsSync3(manifest))
    return {};
  ...
  for (const entry of repos) {
    ...
    if (entry != null && name != null)
      map2[String(name)] = getProp(entry, "root") ?? null;
  }
  return map2;
}
function gatherRows(slug, predicate) {
  const stateRoot = resolveStateRoot(die2);
  const rows = [];
  for (const runDir of listRunDirs(stateRoot, slug)) {
    const delivDir = join2(runDir, "deliverables");
    if (!existsSync3(delivDir))
      continue;
    const runSlug = basename(runDir);
    let repos = null;
    const files = readdirSync(delivDir).filter((f) => f.endsWith(".md")).sort();
    for (const name of files) {
      const file = join2(delivDir, name);
      let fm;
      try {
...
var COLUMNS = ["slug", "id", "status", "repoRoot", "worktree", "branch", "pr", "file"];
function emitRows(rows, lines) {
  if (!lines) {
...
function cmdSyncRows(args) {
  const { slug, lines } = parseGatherArgs(args);
  emitRows(gatherRows(slug, (fm) => fm.status === "pr-open"), lines);
}
function cmdStaleWorktrees(args) {
  const { slug, lines } = parseGatherArgs(args);
  emitRows(gatherRows(slug, (fm) => fm.status === "merged" && fm.worktree != null), lines);
}
var USAGE = "usage: state.mjs <resolve|dag|set|transition|manifest-status|snapshot|sync-rows|stale-worktrees> ...";
...
switch (cmd) {
  ...
  case "snapshot": {
    ...
    const runDirArg = rest[0];
    if (!runDirArg)
      die2("usage: state.mjs snapshot <runDir> [-m <message>]");
