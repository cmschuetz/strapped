...
function parseFeedbackSynthArgs(value) {
  ...
  if (value.lite !== undefined) {
    if (typeof value.lite !== "boolean")
      throw new Error('config: stageArgs["feedback-synth"].lite must be a boolean');
...
async function feedbackSynthStage(cfg) {
  ...
  const task2 = lite ? `2. Return the routed digest ONLY — do NOT append \`## Feedback addendum\` sections and write NO files. The main agent will produce the user-approved plan from this digest.` : `2. Append a \`## Feedback addendum\` section to each affected deliverable's file at ${cfg.dir}/deliverables/<id>-*.md. The section lists concrete, in-scope fix tasks (imperative, testable), each referencing the originating comment/PR. Keep the file conformant to ${cfg.conventionsFile}. If a deliverable already has a \`## Feedback addendum\` section from a prior feedback pass, append new tasks under it rather than duplicating the heading.`;
