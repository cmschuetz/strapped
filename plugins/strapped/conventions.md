...
3. **Plan mode.** The main agent calls **`EnterPlanMode` FIRST** for native plan-mode UX, then reads the digest + every in-scope deliverable plan + the existing code across the worktrees, plans a cross-deliverable refactor, resolves ambiguity via **AskUserQuestion**, and presents the plan via **ExitPlanMode** — the user-gate that replaces feedback's adversarial review + explicit-approval gate. The "no code edits before approval" guarantee is **harness-enforced, not a guideline**: the `UserPromptExpansion` hook (`scripts/plan-lock.sh set`) locks the session the instant `/strapped:feedback-lite` is invoked, and the `PreToolUse` hook (`plan-lock.sh guard`) denies every edit tool until Step 4 releases it — so a model that skips `EnterPlanMode` still cannot edit before approval. `EnterPlanMode` is a model-invoked tool (a guideline on its own); the hook lock is the unbypassable floor beneath it.
4. **Release + approve.** On approval the skill's FIRST action clears the lock (`plan-lock.sh clear <slug>` — session-keyed by the `session_id` in the hook stdin, slug stored as the lock contents so `clear` finds it without knowing the session id), else the `PreToolUse` guard denies the tweak-edits themselves. Fail-closed: an uncleared lock blocks edits (safe, recoverable) rather than ever letting an unapproved edit through. Then apply the user's tweaks directly (no subagents) and append generalizable corrections to `critiques/user-critiques.md` (`synthesized: false`). `--dry-run` still clears the lock (the invocation set it regardless), presents the plan and would-run commands, then stops — mutating nothing else.
