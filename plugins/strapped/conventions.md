...
- **Done-or-later pr gate.** `pr` requires every in-scope node at `done`/`pr-open`/`merged` — the whole run by default, the one named node under `stageArgs.pr.only`: chained after `implement` WITH the same scope (`stageArgs.implement.only` equals `stageArgs.pr.only`, both possibly absent), that stage's `allDone` already proved it; otherwise (pr first, dispatched without implement, or a mismatched-scope chain) a fresh `state.mjs dag` probe (`--only <Did>` when scoped) must show `remaining: 0`, else the stage returns `gateFailed: true` naming the not-done nodes and the dispatch stops.
...
- Branch: `strapped/<slug>/<Did>-<kebab>`; base = parent deliverable's branch **when the parent is in the same repo** (roots, and cross-repo children: `main` — see below). When the same-repo parent is already `merged` at branch-creation time, the child branches from `main` (the parent's work is in main; its pre-merge branch is a dead tip): the implement stage overrides the frontmatter `base:` with the repo's main (after `git fetch origin main`) and records the effective base back to the node's frontmatter — matching the Stacked PRs merged-parent base rule.
...
Topological order over `done` deliverables. Per node: push/create in the **deliverable's own repo** — `git -C <deliverableRepoRoot> push -u origin <branch>`, then `gh pr create --head <branch> --base <parent-branch|main>` in that repo. Parent→child branch stacking only applies **within a single repo**; a cross-repo child roots on `main`, so its PR bases on `main` (not the parent branch) — no cross-repo rebase. **Merged-parent base rule**: a same-repo parent whose status is already `merged` has its work in main and its local branch is a dead pre-merge tip — the child's PR bases on the repo's **main**, with the child branch rebased onto main first when it still carries the parent's pre-merge commits: `git -C <repoRoot> fetch origin main`, then INSIDE the child's worktree `git -C <childWorktree> rebase --onto origin/main <parent-branch>` (parent ref from the merged parent node's `branch:` frontmatter — never the child's `base:`, which the implement stage rewrites to `main` on a resume after the parent merged; when that branch ref was deleted after merge, fall back to the parent's pre-merge tip SHA via `gh pr view <parent-pr> --json headRefOid`), then push `--force-with-lease` — never a checkout-requiring `git rebase main <branch>` from outside the worktree. Parents at `done`/`pr-open` keep the parent-branch base.

**Scoped create** (`stageArgs.pr.only <Did>` / `/strapped:pr <slug> --only <Did>`): candidates are restricted to the named node PLUS any of its ancestors that are `done` without a `pr:` URL (stack coherence — a scoped ship never skips an unshipped done parent), still in `topo` order and still under the parents-done/pr-open/merged candidate rule and the base rules above; the stack-table refresh covers only the PRs the scope touches. The PR title and body follow the [PR titles and bodies (Conventional Commits)](#pr-titles-and-bodies-conventional-commits) spec above: a `<type>(<slug>): <description>` title, then the body's summary + ACs + a `Stack:` table of the whole DAG with PR links + `Depends on #<parent-PR>` for same-repo non-roots. The `Stack:` table may span repos; group or label rows by repo. Record the URL in frontmatter, set status `pr-open`. `--dry-run` prints every git/gh command and PR body without pushing anything.
