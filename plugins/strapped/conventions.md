...
Performed by the skill at review time (the hierarchy may grow). This is an **LLM judgement step, never a script** — discovery, traversal, and imperative extraction are all done by the extracting agent, not by tooling.

1. Discover, from three bounded sources:
   - **CLAUDE.md hierarchy** — every `CLAUDE.md` in the repo (exclude `.venv`, worktrees, `node_modules`) plus `~/.claude/CLAUDE.md` if present.
   - **Skill traversal (progressive disclosure).** After discovering every `CLAUDE.md`, recurse: follow the skills each `CLAUDE.md` loads or points to (progressive-disclosure references, the `SKILL.md` files it names), read them, and extract their normative imperatives too. Keep following transitive references until no new rule-bearing file appears. Record each such rule's source as the **skill file it came from** (the skill path, not the CLAUDE.md that loaded it). This is an LLM judgement step — no script.
   - **Non-CLAUDE.md guideline sources — a FIXED allowlist, never a whole-repo doc sweep** (the guard against large-repo explosion). At plan time, also discover guideline-bearing files matching this allowlist alongside the CLAUDE.md hierarchy: `CONTRIBUTING.md`, `AGENTS.md`, `.cursor/rules/*`, `.github/copilot-instructions.md`, and root- or `docs/`-level files whose names match `STYLE*`, `*GUIDELINES*`, `CONVENTIONS*`, `ARCHITECTURE*` (exclude worktrees, `.venv`, `node_modules` as with CLAUDE.md). Extract only imperatives (skip non-normative prose — the same bar as CLAUDE.md rules); source = the file path. This is the allowlist and nothing beyond it.
...
**Touched-code-scoped discovery (code-review time, diff-bounded).** Touched files are unknowable at plan time, so directory-local guideline docs are discovered at code-review time instead: the code reviewer inspects the directories of the files in `git diff base...branch`, looks for guideline docs (`CONTRIBUTING`/`STYLE`/`README`/`AGENTS`) **in or above** those directories, and applies their rules — **bounded to the diff's touched directories, never a repo sweep**. These diff-scoped rules are reviewer-discovered and are NOT part of the seeded `rulesByRound` split; both reviewers apply whatever they find.

**Acceptance criteria & addenda are first-class rules.** Beyond the assigned-guideline `rule_checklist`, every reviewer in both loops returns a dedicated **`ac_checklist`** covering the artifact's OWN normative items — the plan's `## Acceptance criteria` in the plan flow, the `## Feedback addendum` tasks in the feedback flow. The reviewer reads that section from the artifact files it already opened and enumerates each item itself as `AC1..ACn` / `FA1..FAn`, returning a per-item `pass|violation|na` verdict at the **same weight** as a guideline-rule violation. The `ac_checklist` is a REQUIRED schema field (may be empty when nothing is in scope), so the AC/addendum verdict can never be silently omitted. Unlike guideline rules, it is **not partitioned** by the seeded split — both reviewers cover the full AC/addendum set every round, because it is the artifact's own contract, not partitioned entropy. This hardening matters most for `/strapped:feedback`, where addenda historically "didn't stick": it is enforced at BOTH the addendum-text review (plan-review loop) AND the code-review of the implemented fix.
...
- Code review — **A: correctness** (logic bugs, edge cases, race conditions, error paths, AC compliance, and unreachable/orphaned code paths left behind after changes — code no longer reached by any caller or acceptance criterion). **B: convention & test fidelity** (guideline adherence, test quality: integration-style, no mocking, aiohttp test servers, polyfactory; plus dead or orphaned tests, including tests asserting against removed behavior or covering code that no longer exists).

Both code reviewers additionally enumerate the deliverable's `## Acceptance criteria` (and, in feedback/addendum mode, its `## Feedback addendum` tasks) into the mandatory `ac_checklist` and verify each is implemented in the diff — see [Rule extraction](#rule-extraction).
...
3. **Adversarial review** the synthesized addenda through the SAME machinery — the shared review loop inside `strapped-run.js` (the `feedback-synth` stage runs synthesis, then that loop). Reviewers/refute/dedup/consolidate/revise + the final-round confirmation pass run bounded by the budget. Records land as `reviews/feedback-round-<N>.md` — **distinct from** the original run's `plan-round-<N>.md`. Reviewers enumerate each deliverable's `## Feedback addendum` tasks into the mandatory `ac_checklist` as `FA1..FAn` (per [Rule extraction](#rule-extraction)) so every addendum task gets a per-item verdict at the same weight as a guideline rule — the mechanism that makes addenda "stick".
...
5. **Implement in topological (stack) order** so a parent's fixes land before children rebase. When an addendum supersedes earlier work, the fix path must **remove the code and tests it orphans** — the code-review dead-code lens (see [Reviewer lenses](#reviewer-lenses)) flags leftovers, and in addendum mode the code reviewer also enumerates the `## Feedback addendum` tasks (`FA*`) into the `ac_checklist` and verifies each landed in the diff. Reuse the `implement` stage of `strapped-run.js` on each affected deliverable's EXISTING branch/worktree via its `addendumMode` seam (the implementer APPLIES the `## Feedback addendum` to the existing code instead of re-implementing; the coordinator drives the feedback re-entry transitions and processes one topological rank per wave). Cross-pass progress is tracked by the **workflow itself**, not read off disk: each coordinator pass receives a progress ledger of the ids already processed in THIS dispatch (split done vs parked), the wave is the next topological rank of the affected set counting only un-ledgered nodes, and `remaining` is the count of affected nodes not in the ledger's done list. On-disk markers are NEVER treated as addendum-applied signals — `feedback_rounds_used` and existing `-feedback` records can be left over from a prior feedback batch, and the feedback lifecycle is status-neutral (a node returns to its pre-addendum status), so an empty ledger (pass 1) means every affected node's addendum is unapplied. Thread `recordSuffix: "-feedback"` so feedback code-review rounds write `<Did>-code-round-<N>-feedback.md` (not clobbering the original `<Did>-code-round-<N>.md`); the fix agent's round-record READ path derives from the SAME suffix so writer and reader agree. Freeze rule preserved. Never force-push or merge here.
