...
1. **Fetch** review comments from every in-scope deliverable PR (default: every deliverable with a `pr:` URL that has review comments or a request-changes/comment review; `--deliverable`/`--pr` narrows). THREE categories: line-anchored review comments (`/pulls/{n}/comments` — capture the full anchored RANGE + block, not a single line: `path`, `start_line`/`original_start_line`, `line`/`original_line`, `start_side`, `side`, and the `diff_hunk` block; a multi-line comment spans `start_line..line`, a single-line one has a null `start_line`), review-SUBMISSION bodies (`/pulls/{n}/reviews` — `state` + `body`, where a CHANGES_REQUESTED reviewer states the overarching problem; fed into synthesis as GLOBAL feedback), and global/issue comments (`gh pr view --json comments`). The anchored `path` + range are carried so synthesis can reassign a comment cross-deliverable and read its surrounding block.
