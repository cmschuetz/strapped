...
All state for one strapped run lives under a per-repo **run root** `<runRoot>/<slug>/`. `<runRoot>` is resolved by the [Config resolution](#config-resolution) algorithm below — by default `<stateRoot>/<repo>/` beneath a shared (often global) state root, or `<stateRoot>/` inside the repo in legacy repo-relative mode:
...
Configuration is split so per-repo settings can live *with* the state instead of inside the worked repo:
...
- **Per-repo config** — carries `validations`, `worktreeRoot`, `provisioning` (never `stateRoot` — its own location depends on it). Generated by `/strapped:plan` on first run and confirmed by the user.

`<repo>` is `basename $(git rev-parse --show-toplevel)`, resolved in the primary checkout only — never inside a worktree, where `--show-toplevel` points at the worktree. (Limitation: two repos with the same basename share a namespace — the same assumption the `worktreeRoot` default already makes.)

### Resolving stateRoot and the run root (first match wins)

1. `$STRAPPED_STATE_ROOT` — explicit, forceful override.
2. repo-local `.claude/strapped-config.json` → `stateRoot` (a repo opting to stay self-contained — back-compat).
3. `~/.claude/strapped.json` → `stateRoot` (the anchor; the new default).
4. `plans/strapped` (default).

Expand a leading `~` to `$HOME`, then:

- **Absolute** result → shared mode. `<runRoot>` = `<stateRoot>/<repo>/`; the per-repo config lives at `<runRoot>/strapped-config.json`, colocated with the runs.
- **Relative** result → legacy repo-relative mode. `<runRoot>` = `<stateRoot>/` inside the repo (`<repoAbs>/<stateRoot>/`), and the per-repo config lives at repo-local `.claude/strapped-config.json`. No `<repo>` segment — state is already repo-scoped.

### Resolving the per-repo config (first match wins)

1. repo-local `.claude/strapped-config.json` (self-contained repo — back-compat).
2. `<runRoot>/strapped-config.json` (colocated default).
