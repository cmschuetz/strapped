...
3. **Plan mode.** The main agent calls **`EnterPlanMode` FIRST** to force the pass into harness-enforced plan mode (mutating tools blocked until `ExitPlanMode` — invoking the command is the entry consent), then reads the digest + every in-scope deliverable plan + the existing code across the worktrees, plans a cross-deliverable refactor, resolves ambiguity via **AskUserQuestion**, and presents the plan via **ExitPlanMode** (native plan mode) — the user-gate that replaces feedback's adversarial review + explicit-approval gate. No code edits before approval (plan mode enforces it).
