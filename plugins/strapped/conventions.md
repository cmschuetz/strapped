...
<stateRoot>/
  runs/<slug>/                   # RUN state — owned by the run, keyed by slug
    ...
    reviews/feedback-routing.json # externalId → routedDeliverableId map (heavyweight feedback marking)
...
**Enumerated AC/addendum checklist (both loops).** Beyond the guideline rules — which are partitioned one half per reviewer — every reviewer enumerates the artifact's OWN acceptance criteria (plan/code review) or `## Feedback addendum` tasks (feedback review) as a per-item checklist and returns it EVERY round. This checklist is NOT partitioned: BOTH reviewers return the full `AC1..ACn` (or `FA1..FAn`) list of pass/violation/na verdicts, and a gap — an item the plan or code fails to satisfy or leaves untested — is a BLOCKING finding at guideline-rule weight. So acceptance criteria and feedback addendums carry the same weight as CLAUDE.md guideline rules and cannot silently fail to "stick"; the per-item verdicts are recorded in every round record alongside the rule checklists.
...
`<runDir>/feedback-index.json` — one index per run, spanning every in-scope PR, keyed by GitHub external id. It lets multi-round feedback loops **dedupe already-addressed comments**: fetch reconciles it against GitHub, synthesis is fed only the still-needed subset, post-fix marking records which comments a round addressed, and the pr `--update` write-back resolves them upstream. It is a **plain JSON collection** (not frontmatter — the js-yaml helpers are for `.md` files only), read/written exclusively through `state.mjs feedback-index` (skills never hand-edit it).
...
1. **Fetch** review comments from every in-scope deliverable PR (default: every deliverable with a `pr:` URL that has review comments or a request-changes/comment review; `--deliverable`/`--pr` narrows). THREE categories:
   - **Line-anchored review comments** via the GraphQL **`reviewThreads`** query (NOT the REST `/pulls/{n}/comments` endpoint, which has no resolution field). Fetch **ALL** threads — resolved and unresolved alike — carrying each thread's `isResolved` onto its comments as the index field `githubResolved`, and its node `id` as `threadId`, and the first comment's `databaseId` (stringified) as `externalId`. Capture the full anchored RANGE + block, not a single line: `path`, `startLine`/`originalStartLine`, `line`/`originalLine`, the `diffSide`, and the `diffHunk` block; a multi-line comment spans `startLine..line`, a single-line one has a null `startLine`. Do **NOT** drop resolved threads at fetch: a thread a reviewer self-resolved must still reach upsert, or its entry stays `unaddressed` forever and is re-fed every round. `isOutdated` is deliberately **ignored** (outdated ≠ addressed — the whole point of the index).
   ...
   The anchored `path` + range are carried so synthesis can reassign a comment cross-deliverable and read its surrounding block. Write all fetched records to a scratch file under `<runDir>/reviews/`, then **upsert then cross-check**: `state.mjs feedback-index upsert <runDir> --from <scratch>` (reconciles upstream-resolved threads to `resolved`), then `state.mjs feedback-index read <runDir>` and pass **only** the `status: unaddressed` comments into synthesis. If nothing is unaddressed, report "no outstanding feedback" and stop.
2. **Synthesize cross-deliverable addenda** — ONE consolidated plan, fed only the `unaddressed` comments. A comment left on one PR can produce a fix task on a DIFFERENT deliverable (routed via each deliverable's `Files to touch` map); each fix is appended as a `## Feedback addendum` section on the CORRECT **existing** deliverable file. **No new deliverables, branches, or worktrees are minted.** Because the `feedback-synth` stage/schema are frozen (they return only per-deliverable prose addenda, never `externalId`-keyed structure), a skill-side **routing pass** — a fresh subagent, off the main context — reads the synthesized `## Feedback addendum` sections plus the `unaddressed` records and records an explicit `[{externalId, routedDeliverableId}]` array to `reviews/feedback-routing.json`, routing each comment to the addendum task that addresses it (falling back to the same anchored-`path`→`Files to touch` rule synthesis uses, so the map agrees by construction; `review:`/`issue:` entries route to the deliverable their body/global point was folded into). Every `unaddressed` comment fed to synthesis gets exactly one `routedDeliverableId`.
3. **Adversarial review** the synthesized addenda through the SAME machinery — the shared review loop inside `strapped-run.js` (the `feedback-synth` stage runs synthesis, then that loop). The find → verify-consolidate → act cycle (2 reviewers, then one batch verify-consolidate agent that adjudicates every finding, dedups, and writes the round record, then the reviser) + the final-round confirmation pass run bounded by the budget. Records land as `reviews/feedback-round-<N>.md` — **distinct from** the original run's `plan-round-<N>.md`.
...
1. **Fetch** review comments from every in-scope deliverable PR (default: every deliverable with a `pr:` URL and review feedback; `--deliverable`/`--pr` narrows), the same THREE categories as the Feedback loop's Step 1 (`gh`) — the GraphQL `reviewThreads` query for ALL threads (resolved and unresolved, carrying each thread's `isResolved` as `githubResolved` and its node id as `threadId`; `isOutdated` ignored), plus review-submission bodies under `review:<id>` and global/issue comments under `issue:<id>` (both `threadId: null`, `githubResolved: false`). Write the raw records to a scratch file under `<runDir>/reviews/`, then **upsert then cross-check**: `state.mjs feedback-index upsert <runDir> --from <scratch>`, then `state.mjs feedback-index read <runDir>` and carry only the `status: unaddressed` comments forward (empty ⇒ report "no outstanding feedback" and stop). No `reviews/rules-snapshot.md` / `rulesByRound` is needed — there is no adversarial loop.
2. **Synthesize off-context.** Dispatch the mono-workflow's `feedback-synth` stage ONCE with `lite: true`, passing only the `unaddressed` comments. In lite mode the synthesis agent (running in a subagent, off the main agent's context) returns the routed digest ONLY — it writes NO `## Feedback addendum` files and the stage SKIPS the review loop, returning `{converged: true, rounds: 0, outstanding: [], addenda, summary}`. Heavy reasoning stays off-context so the main agent (the implementer) keeps a lean context. No separate routing file is needed here — the main agent routes every comment in-context via native plan mode (Step 3), so it knows each comment's target node for the Step 5 marking.
...
5. **Implement in the standard loop.** No adversarial code review. In topological (stack) order over the affected nodes, work in each deliverable's EXISTING worktree/branch (mint nothing new), apply the approved changes, run that repo's `validations` until green, and commit. State is recorded through the SAME feedback re-entry edge `pr-open → fixing ⇄ in-review → pr-open` (a pre-PR node with `pr:` null stays `done` — no `done>fixing` edge) and the same `feedback_rounds_used` counter. **After each node's commit, mark the index**: for the comments the main agent routed to that node, `state.mjs feedback-index set <runDir> <externalId> addressed --commit <sha>` with the node's `git rev-parse HEAD` (covering its `review:`/`issue:` entries too); comments routed to a **parked** node stay `unaddressed`. Freeze rule preserved; never force-push or merge.
