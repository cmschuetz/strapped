...
- `addendumMode: true` swaps the implementer's prompt to "apply the `## Feedback addendum` section to the EXISTING code on this branch, staying in scope" — a targeted change, NOT a re-implementation — and switches the coordinator/applier to the feedback re-entry lifecycle: `pr-open → fixing ⇄ in-review → pr-open` (a pre-PR node converges to `done`), never back through `pending`/`ready`/`in-progress`, every flip via the guarded `state.mjs transition`. A node that parks gets `parked` + `parked_reason` and is reported — never forced through.
