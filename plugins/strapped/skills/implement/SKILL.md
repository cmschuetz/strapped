...
The workflow returns `{slug, stages, completed, stoppedAt, results}` — read `results.implement` for `{outcomes, allDone, blocked}`. With `--only <Did>`, `allDone` is scope-relative: it means the SCOPED node is done, not the whole run — report it that way. Summarize: done nodes (with branches and rounds used), parked nodes (with `parkedReason` and the resume command `/strapped:implement <slug> --only <Did>`), blocked-pending children (the `blocked` list), and accumulated non-gating `suggestions` worth a human glance. If `allDone` is true, remind the user of `/strapped:pr <slug> --dry-run` (which may carry the same `--only <Did>` for a scoped ship). If the user gives corrective feedback here that generalizes, append it to `critiques/user-critiques.md` (`synthesized: false`).
