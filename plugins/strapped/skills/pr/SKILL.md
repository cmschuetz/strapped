...
- When a PR is merged externally, a later invocation should notice via `gh pr view --json state` and flip the frontmatter with `node $PLUGIN_ROOT/scripts/state.mjs transition <deliverableFile> merged`. On that flip, mirror the sync hook's cleanup (conventions' **Cleanup recipe**): remove the merged deliverable's worktree + local branch in its own repo (`git -C <repoRoot> worktree remove …` then `branch -D …`, never `--force` — leave a dirty worktree for manual removal) and clear the `worktree` field to `null`. This all happens automatically at session start too: the plugin's SessionStart hook runs `scripts/sync-prs.sh`, which performs the same idempotent flip + worktree/branch cleanup (and sweeps any already-merged deliverable whose worktree was never cleaned up).
