...
| `/strapped:pr <slug> [--dry-run] [--update]` | Stacked PRs via git + gh: child PRs based on their parent deliverable's branch, Conventional-Commits titles (`<type>(<slug>): <description>`) and dependency-annotated bodies; `--update` rebases children after parent changes |
