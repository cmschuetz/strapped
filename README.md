...
|---|---|
| `/strapped:plan <plan.md> [--repo <path-or-name>]... [--seed N] [--max-rounds N]` | Research → DAG of theme-scoped deliverables → adversarial plan-review loop (2 reviewers with disjoint CLAUDE.md rule halves + distinct lenses, then one batch verify-consolidate pass adjudicating every finding and deduping vs seen, bounded at 1 round by default — `--max-rounds` overrides, `0` skips review) → interactive final review. Fresh runs get a truly random seed recorded in the manifest (`--seed N` forces one). `--repo` names the target repo(s) the work lands in (repeatable); omit to be prompted. A run may span multiple repos. |
...
The strapped operating context loads itself: a SessionStart hook runs `plugins/strapped/scripts/preamble.sh`, which injects the **slim** `plugins/strapped/context.md` operating model — not the full conventions — plus a live state summary (every run under `<stateRoot>/runs/` with its manifest status and per-status deliverable counts) into the orchestrator's context. `context.md` carries what the orchestrator needs from the start (what strapped is, the 7 skills and when to run each, the core mental model, and a disclosure map); the deep formats stay in `conventions.md` and are read **on demand**, section-by-section, only when a skill points at the exact format it needs — progressive disclosure, never front-loaded. The injection's first line carries the sentinel `strapped-preamble-v1`. It fires on `startup`, `clear`, and `compact` — deliberately NOT on `resume`, because a resumed session's transcript already contains the earlier injection (clear/compact evict it, so those re-inject) — and never fires for subagents (workflows still seed subagents with the full `conventions.md` explicitly). Skills assume the preamble is present and fall back to reading `context.md` themselves only when the sentinel is missing from context.
...
When reviewers request changes on a run's stacked PRs, `/strapped:feedback <slug>` closes the loop back into the run — the **"chisel"** pass for a quick, user-observed polish. It fetches every in-scope PR's review comments via `gh` — line-anchored comments, review-submission bodies (including `CHANGES_REQUESTED` summaries), and global comments — then puts you in the loop via Claude's native plan mode: the main agent synthesizes ONE consolidated **cross-deliverable** fix plan **in-band** (a comment left on one PR can land a fix on a different deliverable — no subagent, no workflow dispatch), asking questions to resolve ambiguity and presenting the plan via ExitPlanMode for you to tweak/approve. It then implements the approved plan in the standard loop on each deliverable's EXISTING branch/worktree in stack order (no new branches, no force-push, no merge, no adversarial loops), reusing the `pr-open → fixing ⇄ in-review → pr-open` re-entry edge and the separate `feedback_rounds_used` counter so the original run's audit stays intact. `--dry-run` stops after presenting the plan. The run ends by offering `/strapped:pr <slug> --update` once to rebase the stack.
