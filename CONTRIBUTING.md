...
**Scenario tests ship with the behavior they test.** A deliverable that changes workflow behavior includes the scenario eval(s) exercising that behavior in the SAME deliverable/PR — never as a follow-up. The scenario is part of the change's definition of done: without it the baseline-vs-candidate compare below has nothing to bite on for the new behavior.
