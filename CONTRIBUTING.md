...
**Scenario evaluations.** The same `src/eval/` tree also carries *scenario* evals (`src/eval/scenarios/harness/`, run via `bun run eval --scenarios src/eval/scenarios/harness`): each scenario materializes a sandboxed synthetic fixture repo + ask, drives the REAL shipped workflow deployable (`plugins/strapped/workflows/strapped-run.js`) through a stage subset with every `agent()` call lowered into a real `claude -p` run, and grades the whole run on **correctness**, **adherence** (the workflow's own state/frontmatter rules), **time**, and **price**. Run them when changing **LLM-facing** workflow behavior — agent prompts, or orchestration semantics the agents must follow (what a prompt asks an agent to decide or produce) — and capture a `--json` report before and after the change: the offline `bun run eval --compare <baseline.json> <candidate.json>` diff is the before/after evidence for such a change. Purely mechanical plumbing — gates, config parsing, arg threading, scoping — changes nothing LLM-wise: hermetic `bun test` coverage is the required evidence there, and no scenario run is owed. Scenarios are much heavier than prompt cases: every agent call is a real multi-turn agentic run, so expect minutes of wall clock and real cost per scenario (keep asks tiny; the pr stage is always forced dry-run). Known limitation: the engine's sync spawn boundary serializes the workflow's `parallel()`/`pipeline()`, so scenario wall clock is the SERIALIZED sum of agent calls — rest time verdicts on Σturns / ΣapiDurationMs deltas, never on the wall column.

**Scenario tests ship with the behavior they test.** A deliverable that changes **LLM-facing** workflow behavior (per the scope above) includes the scenario eval(s) exercising that behavior in the SAME deliverable/PR — never as a follow-up. The scenario is part of the change's definition of done: without it the baseline-vs-candidate compare below has nothing to bite on for the new behavior. A mechanical-plumbing deliverable owes no scenario.

**Per-node baseline-vs-candidate runbook.** When a branch changes LLM-facing workflow behavior, its merge evidence is a baseline-vs-candidate compare over the scenario suite, and the baseline MUST come from a **self-consistent checkout** — the old executor, old scenarios, and old deployable together, run from INSIDE a worktree of the baseline commit. Per node:
